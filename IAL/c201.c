...
void List_Dispose( List *list ) {
    ListElementPtr current_ptr = list->firstElement;
    // Abych neztratil adresu tak vyuziju pomocneho ukazatele

    while (current_ptr != NULL){
        list->firstElement = current_ptr->nextElement;
        free(current_ptr);
        current_ptr = list->firstElement;
    }
}
...
void List_InsertFirst( List *list, int data ) {
    /*
     * Tady jsem narazil na velmi zajimavy problem.
     * Reseni co mi funguje na local (Ubuntu 20.04.5 LTS bezici na WSL) mi hazi na obou serverech segfault.
     * Reseni co mi funguje na Eve a i na Merlinovi se mi zase neprelozi na local, protoze pry je to incomplete struct.
     */

    ListElementPtr element = malloc(sizeof(ListElementPtr)); // Takto me to funguje na local Ubuntu 20.04.5 LTS
    // ListElementPtr element = malloc(sizeof(struct ListElement)); // Takto me to funguje na Eve a Merlinovi
...
void List_DeleteFirst( List *list ) {
    ListElementPtr ptr = list->firstElement;
    if (ptr == NULL) return;
    list->firstElement = ptr->nextElement;
    
    if (list->activeElement == ptr) {
        list->activeElement = NULL;
    }
...
void List_DeleteAfter( List *list ) {
    ...
    ListElementPtr ptr = list->activeElement->nextElement;
    if (ptr == NULL) return;

    list->activeElement->nextElement = ptr->nextElement;
...
void List_InsertAfter( List *list, int data ) {
    if (list->activeElement == NULL) return;
    // Vice info u List_InsertFirst
    ListElementPtr new_ptr = malloc(sizeof(ListElementPtr)); // Takto me to funguje na local Ubuntu 20.04.5 LTS
    // ListElementPtr element = malloc(sizeof(struct ListElement)); // Takto me to funguje na Eve a Merlinovi
    if (new_ptr == NULL) {
        List_Error();
        return;
    ...
    ListElementPtr active_ptr = list->activeElement;
    new_ptr->data = data;
    new_ptr->nextElement = active_ptr->nextElement;
    active_ptr->nextElement = new_ptr;
}
